...
class NotebooksListViewController: UIViewController, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        /*We specify Notebook as type parameter which makes this fetch request that works with specific managed object subclass
          and we call fetch request on that subclass which returns a new fetch request intialized with that entity*/
        let fetchRequest : NSFetchRequest<Notebook> = Notebook.fetchRequest()
        let sortDescriptor = NSSortDescriptor(key: "creationDate", ascending: false)
        fetchRequest.sortDescriptors = [sortDescriptor]
        
        //we need ask a context to execute the request
    ...
    func deleteNotebook(at indexPath: IndexPath) {
        ...
        //use hasChanges before saving every time
        try? dataController.viewContext.save()
